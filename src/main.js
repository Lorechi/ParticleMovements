...
menuTrigger.addEventListener('click', togglePanel);
let last = performance.now(), lastCameraUpdate = 0;
function frame(now) { try { particles.resize(); particles.setCount(settings.count); if (now - lastCameraUpdate > 66) { try { field.update(); } catch (error) { field.ready = false; console.warn('Camera processing paused after an error; particles will continue flowing.', error); } lastCameraUpdate = now; } particles.update(field, now, (now - last) / 1000); particles.render(); } catch (error) { console.error('Particle frame failed; retrying on the next animation frame.', error); } last = now; requestAnimationFrame(frame); }
requestAnimationFrame(frame); field.start();
addEventListener('resize', () => particles.resize()); addEventListener('keydown', async e => { const key = e.key.toLowerCase(); if (key === 'h') togglePanel(); if (key === 'd') { field.showDebug = !field.showDebug; debug.style.display = field.showDebug ? 'block' : 'none'; } if (key === 'r') field.reset(); if (key === 'i') settings.invert = !settings.invert; if (key === 'f') { if (document.fullscreenElement) await document.exitFullscreen(); else await document.documentElement.requestFullscreen(); } });
