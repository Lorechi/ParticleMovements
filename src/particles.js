...
export class Particles {
  constructor(canvas, settings) { this.canvas = canvas; this.settings = settings; this.gl = canvas.getContext('webgl', { alpha: false, antialias: false, powerPreference: 'high-performance' }); if (!this.gl) throw new Error('WebGL is required.'); this.count = 0; this.contextLost = false; canvas.addEventListener('webglcontextlost', event => { event.preventDefault(); this.contextLost = true; console.warn('WebGL context lost; waiting for browser recovery.'); }); canvas.addEventListener('webglcontextrestored', () => { this.contextLost = false; this.initGL(); this.resize(); console.info('WebGL context restored.'); }); this.initGL(); this.resize(); this.setCount(settings.count); }
